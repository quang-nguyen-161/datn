...
#include "filter.h"   /* dc_filter_t, DC_WINDOW, biquad_df2t_t */
...
/* ── Types ── */
typedef enum { max30102_heart_rate = 0x02, max30102_spo2 = 0x03, max30102_multi_led = 0x07 } max30102_mode_t;
typedef enum { max30102_smp_ave_1, max30102_smp_ave_2, max30102_smp_ave_4, max30102_smp_ave_8, max30102_smp_ave_16, max30102_smp_ave_32 } max30102_smp_ave_t;
typedef enum { max30102_sr_50, max30102_sr_100, max30102_sr_200, max30102_sr_400, max30102_sr_800, max30102_sr_1000, max30102_sr_1600, max30102_sr_3200 } max30102_sr_t;
typedef enum { max30102_pw_15_bit, max30102_pw_16_bit, max30102_pw_17_bit, max30102_pw_18_bit } max30102_led_pw_t;
typedef enum { max30102_adc_2048, max30102_adc_4096, max30102_adc_8192, max30102_adc_16384 } max30102_adc_t;
typedef enum { max30102_led_off, max30102_led_red, max30102_led_ir } max30102_multi_led_ctrl_t;
...
typedef struct { uint32_t red; uint32_t iRed; } SAMPLE;

/* ── Ring buffer (100-sample float) ── */
typedef struct { float buff[100]; uint8_t rb_head; } rb_t;

/* ── Driver API ── */
void max30102_write(uint8_t reg, uint8_t value);
void max30102_read(uint8_t reg, uint8_t *dest, uint8_t n);
...
void max30102_cal(void);   /* read FIFO, compute HR/SpO2, update g_ppg */
...
/* ── Timer2 microsecond counter (shared resource) ── */
...
uint32_t spo2_count(dc_filter_t *dc_ir, dc_filter_t *dc_red, rb_t *ir_rb, rb_t *red_rb);

void     ring_buffer_push(uint32_t *buff, uint32_t value, uint32_t size, uint32_t *head);
void     peak_time_push1(uint32_t *buff, uint32_t *delta_t_buff, uint32_t size, uint32_t head, uint32_t *dt_head, uint32_t delta_size);
uint32_t hr_count1(uint32_t *delta_t_buff, uint32_t delta_size, uint32_t peak, uint32_t dt_head);
uint32_t spo2_count1(uint32_t *ir_buff, uint32_t *red_buff, uint32_t size, uint32_t head);
